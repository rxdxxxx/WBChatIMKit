...
static const NSTimeInterval CSToastFadeDuration = 0.6;

// shadow appearance
static const CGFloat CSToastShadowOpacity       = 0;
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    ...
    if ([self isKindOfClass:[UITableView class]]) {
        return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height)) - 124);
    }
    
    
    // default to bottom
    return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height)) - 60);//CSToastVerticalPadding);
}
